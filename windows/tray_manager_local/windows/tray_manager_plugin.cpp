...
void TrayManagerPlugin::PopUpContextMenu(
  ...
  // Standard tray context-menu pattern (MSDN NotifyIcon sample):
  // 1) SetForegroundWindow so the menu can take focus (works when the
  //    window is minimized but not hidden); otherwise clicking outside
  //    won't dismiss the menu.
  // 2) TrackPopupMenu with standard flags; selected item is delivered
  //    via WM_COMMAND, which the plugin forwards to onTrayMenuItemClick.
  // 3) PostMessage(WM_NULL) helps the menu close on outside click.
